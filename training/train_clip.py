...
RESULTS_DIR = BASE_DIR / "results" / "model_2_clip"
...
class CLIPMultimodalModel(nn.Module):
    def __init__(self, text_dim=768, image_dim=2048, proj_dim=512):
        super().__init__()
    ...
    def forward(self, text_feat, image_feat):
        ...
        t_embed = t_embed / (t_embed.norm(dim=-1, keepdim=True) + 1e-8)
        i_embed = i_embed / (i_embed.norm(dim=-1, keepdim=True) + 1e-8)
        fused = torch.cat([t_embed, i_embed], dim=1)
...
def run_5fold_cv(text_feats, image_feats, sid_to_idx):
    print("======================================")
    print("MODEL 2 (CLIP) 5-FOLD CROSS-VALIDATION")
    print("======================================")
    ...
    for fold in range(1, 6):
        ...
        model = CLIPMultimodalModel().to(device)
    ...
    print("\n--- Model 2 (CLIP) 5-Fold CV Summary ---")
    print(summary_df.to_string(index=False))

def train_and_eval_final_test(text_feats, image_feats, sid_to_idx):
    print("\n======================================")
    print("MODEL 2 (CLIP) FINAL TEST EVALUATION")
    print("======================================")
    ...
    metrics = {
        "model": "CLIP Multimodal Sentiment Classifier",
